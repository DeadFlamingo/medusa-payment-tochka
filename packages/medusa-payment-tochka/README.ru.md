...
Также зарегистрируйте плагин, чтобы в Medusa Admin появилась страница настроек:
...
После регистрации плагина в Medusa Admin появляется пункт **Settings → Extensions → Точка**.

Страница сохраняет настройки в записи магазина и применяет их на следующем платеже без перезапуска backend:

- JWT-токен, client ID, версия API и режим Production / Developer
- Назначение платежа, двухстадийная оплата и URL витрины
- Способы оплаты: карта, СБП, рассрочка Т-Банка, Долями
- Чеки 54-ФЗ, система налогообложения и НДС
- Публичный JWK вебхука

Секреты в интерфейсе маскируются. Пустое поле секрета сохраняет текущее значение. Переменные окружения остаются запасным источником, пока страница в админке не будет сохранена.
